...
- 多个初始化接口必须统一使用 `await Promise.all([...])` 并发请求，不使用串行 `await` 逐个请求，也不使用裸 `Promise.all(...).then(...)`。
- `Promise.all` 返回结果的接收变量，统一按“方法名 + \`Res\`”命名，例如 `getCoursesHotRes`、`getStatsOverviewRes`。
...
Page({
  ...
  async initData() {
    ...
    this.setData({
      ...
      heroStats: buildHeroStats(getStatsOverviewRes),
    });
  },
});
```
...
- 如果项目使用自定义 tabbar，则每个 tabbar 页面都必须在 `onShow` 生命周期中从 `@miniprogram/utils/util` 引入并执行 `setTabBarSelected()`，用于同步当前 tab 高亮态；不要放在 `onLoad`、`onReady` 中执行，也不要漏掉任一 tabbar 页面。

## 执行清单

- 先判断任务是否命中“适用场景”；未命中时不要加载本 skill。
- 遇到页面、组件、路由或样式改动时，优先检查新增方法、变量声明、配置项是否已经补上中文注释。
- 遇到页面初始化接口请求相关改动时，优先检查 `onLoad` 是否只调用 `initData`，接口调用是否都落在 `initData` 内，页面是否额外维护了接口请求 loading，是否使用 `await Promise.all([...])`，以及结果变量是否符合“方法名 + Res”命名。
...
- 遇到自定义 tabbar 页面相关改动时，优先检查页面是否已在 `onShow` 中调用 `setTabBarSelected()`，并确认所有 tabbar 页面实现保持一致。
